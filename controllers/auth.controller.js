...
const Entrenador = db.entrenador;
...
exports.login = async (req, res) => {
  try {
    ...
    const activePersonalizacion = await db.entrenadorPersonalizacion.findOne({
      where: { entrenadorId: entrenador.id, estaActivo: true },
      include: [
        {
          model: db.personalizacion,
          as: "personalizacion",
          attributes: ["id", "tipo", "descripcion", "idFoto"],
          where: { tipo: "Foto de Perfil" },
        },
      ],
    });
    ...
    return res.json({
      entrenador: {
        ...
        sexo,
        pokedolares,
        personalizacion: activePersonalizacion
          ? activePersonalizacion.personalizacion
          : null,
      },
      accessToken,
      refreshToken,
...
exports.refresh = async (req, res) => {
  try {
    ...
    const user = await Entrenador.findByPk(payload.sub);
    if (!user)
...
exports.me = async (req, res) => {
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .json({ message: "Token requerido (Authorization: Bearer <token>)" });
    }
